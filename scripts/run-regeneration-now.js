...
class RegenerationRunner {
  ...
  async run() {
    try {
      ...
      // Tenter une sauvegarde, mais continuer même en cas d'échec
      try {
        await this.createBackup();
      } catch (backupError) {
        console.warn(`⚠️ Impossible de créer une sauvegarde: ${backupError.message}`);
        console.warn('Continuons sans sauvegarde...');
      }
      
      // Récupérer les cols
      let cols = await this.colsCollection.find({}).toArray();
      console.log(`${cols.length} cols trouvés.`);
      
      if (cols.length === 0) {
        console.warn('⚠️ Aucun col trouvé dans la collection. Création de données de test...');
        await this.createSampleData();
        console.log('Échantillons de données créés. Récupération...');
        cols = await this.colsCollection.find({}).toArray();
        console.log(`${cols.length} cols de test créés.`);
        
        if (cols.length === 0) {
          console.error('❌ Impossible de créer des données de test. Vérifiez les droits d\'accès à MongoDB.');
          return;
        }
      }
  ...
  async createBackup() {
    try {
      ...
      if (cols.length === 0) {
        console.warn('⚠️ Aucun col à sauvegarder.');
        return;
      }
      ...
      throw new Error(`Impossible de créer une sauvegarde: ${error.message}`);
    }
  }
   ...
   */
  async generateElevationProfile(coordinates) {
    try {
   ...
   */
  calculateCumulativeDistance(points, startIdx, endIdx) {
    let distance = 0;
  ...
  /**
   * Crée des données de test pour les cols
   */
  async createSampleData() {
    try {
      const sampleData = [
        {
          ...
          coordinates: [45.9833, 6.5167]
        },
        {
          name: 'Col de la Forclaz',
          coordinates: [46.0667, 6.7833]
        },
        {
      ...
      await this.colsCollection.insertMany(sampleData);
    } catch (error) {
