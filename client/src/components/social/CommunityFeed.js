...
import { 
  ...
  ListItemAvatar, Collapse, Tabs, Tab, Dialog, DialogTitle, 
  DialogContent, DialogActions
} from '@mui/material';
import { 
  Favorite, FavoriteBorder, Comment, 
  MoreVert, Send, FilterList, Close
} from '@mui/icons-material';
import SocialService from '../../services/socialService';
// Importation de react-icons
import { FiShare, FiThumbsUp, FiMessageSquare, FiBookmark, FiMoreHorizontal } from 'react-icons/fi';
import { FaFacebook, FaTwitter, FaWhatsapp } from 'react-icons/fa';
import { MdContentCopy } from 'react-icons/md';
// Importation de react-share
import { 
  FacebookShareButton, 
  TwitterShareButton, 
  WhatsappShareButton, 
...
const CommunityFeed = ({ userId }) => {
  ...
  // Nouvel état pour la boîte de dialogue de partage
  ...
  const handleImageError = (e) => {
    e.target.src = '/images/social/unknown.svg';
  };
  
  // Fonction pour gérer les erreurs d'avatars
  const handleAvatarError = (e) => {
    e.target.src = '/images/social/default-avatar.svg';
  };
  ...
  // Get share URL for the current post
  const getShareUrl = (postId) => {
    return `${window.location.origin}/posts/${postId}`;
  };

  // Get share title for the current post
  const getShareTitle = (postId) => {
    const post = posts.find(p => p.id === postId);
    if (!post) return 'Publication Grand Est Cyclisme';
    
    // Limiter le contenu à 50 caractères pour le titre
    let title = post.content.substring(0, 50);
    if (post.content.length > 50) title += '...';
    return `${post.user.name}: ${title}`;
  };
  ...
  return (
    <div className="community-feed">
            ...
            <Grid item xs={12} key={post.id}>
              <Card>
                <CardContent>
                  ...
                  <CardActions sx={{ px: 0 }}>
                    <Button 
                      startIcon={post.userLiked ? <Favorite color="error" /> : <FiThumbsUp size={20} />}
                      onClick={() => handleToggleLike(post.id)}
                      color={post.userLiked ? "primary" : "inherit"}
                      size="small"
                    >
                      J'aime
                    </Button>
                    <Button 
                      startIcon={<FiMessageSquare size={20} />}
                      onClick={() => handleExpandComments(post.id)}
                      color="inherit"
                      size="small"
                    >
      ...
      <Dialog 
        ...
        PaperProps={{
          sx: { borderRadius: 2, width: { xs: '90%', sm: '450px' } }
        }}
      >
        <DialogTitle>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <Box display="flex" justifyContent="space-between" width="100%" mb={3}>
                <FacebookShareButton url={getShareUrl(sharePostId)} quote={getShareTitle(sharePostId)}>
                  <IconButton sx={{ color: '#1877F2' }}>
                    <FaFacebook size={32} />
                  </IconButton>
                </FacebookShareButton>
                
                <TwitterShareButton url={getShareUrl(sharePostId)} title={getShareTitle(sharePostId)}>
                  <IconButton sx={{ color: '#1DA1F2' }}>
                    <FaTwitter size={32} />
                  </IconButton>
                </TwitterShareButton>
                
                <WhatsappShareButton url={getShareUrl(sharePostId)} title={getShareTitle(sharePostId)}>
                  <IconButton sx={{ color: '#25D366' }}>
                    <FaWhatsapp size={32} />
                  </IconButton>
                </WhatsappShareButton>
                
                <LinkedinShareButton url={getShareUrl(sharePostId)}>
                  <IconButton sx={{ color: '#0A66C2' }}>
                    <FaLinkedin size={32} />
                  </IconButton>
                </LinkedinShareButton>
                
                <EmailShareButton url={getShareUrl(sharePostId)} subject="Grand Est Cyclisme - Partage">
